...
class ThreePageAdapter(
    ...
    override fun destroyItem(container: ViewGroup, position: Int, `object`: Any) {
        container.removeView(`object` as View) //as 는 캐스팅 개념
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        when(position){
            ...
            1-> {
                val view = layoutInflater.inflate(R.layout.fragmenttwo, container, false)
                container.addView(view)
                return view
            }
            2-> {
                val view = layoutInflater.inflate(R.layout.fragmentthree, container, false)
                container.addView(view)
                return view
            }
            else ->  {
